...
export const incrementIfEvenAsync = createAsyncThunk<number, number, 
{ state: RootState }>(
  "counter/fetchCount",
  async (amount, { getState }) => {
    ...
    if(currentValue % 2 === 0) {
      console.log("Even");
      const response = await fetchCount(currentValue);
      return response.data
    }
    return 0; // Need to return something when condition isn't met
  },
)
